...
inline U16 ub(const bco::Tensor &tensor, U32 flips = 1'000'000);

// The same upper bound together with a decomposition of the input tensor.
inline std::vector<Scheme::Term> ubd(const bco::Tensor &tensor,
                                     U32 flips = 1'000'000);

inline std::vector<Scheme::Term> ubd(const bco::Tensor &input,
                                     U32 flips_limit) {
    ...
    std::vector<Scheme::Term> best_terms = scheme.terms();
    ...
    std::erase_if(best_terms,
                  [](const Scheme::Term &term) { return term[0] == 0; });
    return best_terms;
}

inline U16 ub(const bco::Tensor &tensor, U32 flips) {
